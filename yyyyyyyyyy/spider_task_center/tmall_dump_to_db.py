                    ...
                    for mysqlFile in mysqlFileList:
                        if re.search("^(.*).sql$",mysqlFile):
                            ...
                            for s in lines:
                                sqlResult=None
                                try:
                                    if s.find("update") != -1  and s.find("origin_price") != -1:
                                        s = s.replace("and promot",", promot")
                            #            print s
                                        cursor.execute(s)
                                except:   
                                    print "error===>" + s 
...
#        return
    ...
    #打开你要写得文件pp2.txt
    for s in lines:       
        s = s.replace('\'','\\\'').replace(',\\\'',',\'').replace('\\\',','\',').replace('\\\')','\');').replace('(\\\'','(\'').replace('=\\\'','=\'').replace('\\\' w','\' w').replace('= \\\'','= \'').replace('\\\' a','\' a') 
        fp.write(s)   
        # replace是替换，write是写入
    fp.close()
